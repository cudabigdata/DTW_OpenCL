 ...
 */
inline void upsweep_pow2(__local double *x, int m) {
 ...
 * NB: [m] must be a power of two.
 */
inline void sweepdown_pow2(__local double *x, int m) {
  int lid = get_local_id(0);
  int bi = (lid*2)+1;
 ...
 * NB: m must be a power of two.
 */
inline void scan_pow2(__local double *x, int m) {
  int lid = get_local_id(0);
  int lane1 = (lid*2)+1;
  upsweep_pow2(x, m);
  if (lane1 == (m-1)) {
 ...
 */
__kernel void scan_pow2_wrapper(__global double *data, __local double *x, int m) {
...
__kernel void scan_pad_to_pow2(__global double *data, __local double * x, int n) {
  int gid = get_global_id(0);
  int lane0 = (gid*2);
  int lane1 = (gid*2)+1;
...
__kernel void scan_subarrays(
  __global double *data, //length [n]
  __local  double *x,    //length [m]
  __global double *part, //length [m]
           int n
) {
  // workgroup size
 ...
 * NB: [m] must be a power of two.
 */
inline void fupsweep_pow2(__local float *x, int m) {
  int lid = get_local_id(0);
  int bi = (lid*2)+1;
 ...
 * NB: [m] must be a power of two.
 */
inline void fsweepdown_pow2(__local float *x, int m) {
  int lid = get_local_id(0);
  int bi = (lid*2)+1;
 ...
 * NB: m must be a power of two.
 */
inline void fscan_pow2(__local float *x, int m) {
  int lid = get_local_id(0);
  int lane1 = (lid*2)+1;
  fupsweep_pow2(x, m);
  if (lane1 == (m-1)) {
 ...
 *     there must be exactly one workgroup of size m/2
 */
__kernel void fscan_pow2_wrapper(__global float *data, __local float *x, int m) {
  int gid = get_global_id(0);
  int lane0 = (gid*2);
  int lane1 = (gid*2)+1;
...
__kernel void fscan_pad_to_pow2(__global float *data, __local float * x, int n) {
  int gid = get_global_id(0);
  int lane0 = (gid*2);
  int lane1 = (gid*2)+1;
  int m = 2*get_local_size(0);
 ...
 * The last subarray will be padded with 0 if necessary (n < k*m).
 * We use the primitives above to perform a scan operation within each subarray.
 * We store the intermediate reduction of each subarray (following fupsweep_pow2) in [part].
 * These partial values can themselves be scanned and fed into [scan_inc_subarrays].
 */
__kernel void fscan_subarrays(
  __global float *data, //length [n]
  __local  float *x,    //length [m]
  __global float *part, //length [m]
           int n
) {
  // workgroup size
  int wx = get_local_size(0);
 ...
 * We sum each element by the sum of the preceding subarrays taken from [part].
 */
__kernel void fscan_inc_subarrays(
  __global float *data, //length [n]
  __local  float *x,    //length [m]
  __global float *part, //length [m]
           int n
